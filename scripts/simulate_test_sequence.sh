...
poetry run pytest tests/unit --junitxml=$UNIT_JUNIT --cov=src --cov-report=xml:$UNIT_COV --alluredir=allure-results/unit

# Run smoke tests
SMOKE_JUNIT=junit-smoke.xml
SMOKE_COV=coverage-smoke.xml
echo "Running smoke tests..."
poetry run pytest tests/smoke --junitxml=$SMOKE_JUNIT --cov=src --cov-report=xml:$SMOKE_COV --alluredir=allure-results/smoke
