...
@pytest.mark.parametrize("Launcher, browser_name, config, expected_args, headless_flag", [
    # Chrome
    (
        ChromeLauncher,
        "chrome",
        ...
        ["/usr/bin/google-chrome", "--headless", "--user-data-dir=/tmp/profile", "--test-flag", "https://example.com"],
        "--headless"
    ),
    # Firefox
    (
        ...
        ["/usr/bin/firefox", "-headless", "-profile", "/tmp/profile", "--test-flag", "https://example.com"],
        "-headless"
    ),
...
def test_launcher_instantiation_and_args(Launcher, browser_name, config, expected_args, headless_flag):
